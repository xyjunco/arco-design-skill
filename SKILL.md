...
当无法填写 `chosen_component` 或 `required_properties` 时，不允许继续手绘；必须先检索组件或记录组件库缺口。
...
- 新建项目、选择模板、配置模板时，如果节点之间存在依赖关系，必须展示依赖预览和影响范围；不能只用平铺 Checkbox 或普通列表。
...
59. 模板选择、新建项目、节点选择或类似向导是否展示了依赖预览和取消/反选影响，而不是只有平铺 Checkbox？
