...
21. Select/Input/Button 等控件内容是否写入组件 `TEXT` property 或实例内部文本层，而不是外部覆盖层？
22. 画布中是否不存在可见或隐藏的 `Select cover`、`Input cover`、`Button text cover`、`Text / ⌄`、`fake-*`、`overlay-*` 等伪装控件层？
23. 表单 label/control 是否在同一栅格内，且没有 label 与控件重叠？
24. 表格列宽总和是否严格小于容器内容宽度？
25. Pagination 是否因为 page-size selector 或跳转输入越界？
26. Modal/Drawer 内是否存在低透明度组件残影、双文本源或区块标题重叠？
27. 是否已用截图抽检列表、表单、弹窗、详情四类页面？
28. 列表页顶部操作、指标卡、筛选栏、表格、分页是否共享同一内容右边界？
29. Pagination 实例宽度是否覆盖内部完整页码结构，且少量数据时已吸附到列表底部？
30. 表格中的业务域、等级、短状态 Tag 是否按内容收窄，而不是拉满单元格？
31. 范围/过滤/DSL 输入是否已优先拆成字段、操作符、值域和加删行，而不是纯 textarea？
32. 条件构建器是否根据字段类型选择 select、Tag 多选、InputNumber、radio/switch 或时间控件？
33. 表达式/模板编辑区附近是否提供可复制的变量清单，并解释每个变量的含义？
34. 图表区域是否已搜索并使用 `源力设计体系-图表库`，而不是手绘折线、柱、饼、图例、坐标轴或 tooltip？
35. 图表类型是否匹配用户任务：趋势=折线/折线组合，规模=面积，对比=柱状，排行=Toplist，少量构成=饼/环形？
36. 图表的 `数量 #of lines`、`线数量`、`分类数量 Item` 是否和真实系列/分类数量一致？
37. 图表里是否没有残留 `P90/P95/Item 1/我是标题` 等默认样例文案？
38. 图表 hover/focus/State 是否只用于交互态，没有被拿来表达业务状态？
39. 每个 top-level 页面 frame 是否已按最终可见内容重新收口或增高，并保留 64-96px 底部空白？
40. Card、Modal、Drawer、Popover、Confirm、Alert 是否包住内部可见内容，没有表格、提示、按钮或文本浮在容器外？
41. 表格外框、直接列 cell、卡片内容区、页面 contentRight 是否同时对齐，没有最后一列伸出或被裁切？
42. 图表库组件是否有足够 viewport，趋势线、条形、图例、行标签、数值和 tab 是否完整可见？
43. 若使用图表裁切，裁切容器是否明确命名、设置 `clipsContent=true`，且只裁掉组件内部无关默认区域而非业务信息？
44. 批量生成或修复后，是否已用 Figma API 检查所有页面 frame 和关键容器的 `absoluteBoundingBox` 溢出？
45. 是否已区分“图表 viewport 的有意裁切”和“普通容器布局错误”，没有用 mask/clipping 掩盖表格或 Alert 溢出？
46. 截图 QA 是否覆盖至少一个图表页，并确认图表没有默认样例文案、图例颜色不匹配、分布行不完整或被裁切？
47. Select 是否是真实 `select` 实例，选中值写入实例内部或文本属性，箭头来自组件自身而不是手写字符/外部 icon？
48. Input/SearchBox 是否是真实组件实例，placeholder/value/search icon 来自组件自身，没有 `Input cover` 或手绘搜索 icon？
49. Button 文案和颜色是否来自 Button 实例属性或内部文本层，没有空按钮底板加外部文本？
50. Dropdown、Cascader、TreeSelect 展开态是否使用对应菜单/选项组件，而不是 Select + 手写选项拼装？
51. 控件区域是否只有一个业务文本来源，没有组件内部文本和 sibling 文本同时存在？
52. 批量替换组件文本后，是否检查了内部文本层的 fill、opacity、font、auto-resize 和 bounding box，确认不是“结构正确但截图不可见”？
